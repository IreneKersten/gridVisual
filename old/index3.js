// sankey with positive and negative blocks
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    d3.csv("data4.csv").then(function(csvdata) {
        ...
        const sankey = d3.sankey()
         ...
         sankey.linkSort((a,b) => a.realValue - b.realValue );
        ...
        nodes.forEach(function(node) {
            ...
            node.sourceLinks.forEach(function(sourceLink){
               if( sourceLink.extraCat == 'pos' ) {
                posNodeVal += sourceLink.value
               } else {
                negNodeVal += sourceLink.value
               }
            })

            node.targetLinks.forEach(function(targetLink){
                if( targetLink.extraCat == 'pos' ) {
                 posNodeVal += targetLink.value
                } else {
                 negNodeVal += targetLink.value
                }
             })

            node.posVal = posNodeVal;
            node.negVal = negNodeVal;
            node.value = posNodeVal + negNodeVal;

            node.fixedValue = 3;
           const  valuePercentageRect1 = 1 -(posNodeVal / node.value);
           const posPixelEnd = valuePercentageRect1 * (node.y1-node.y0) + node.y0
